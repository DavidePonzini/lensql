...
CREATE OR REPLACE FUNCTION remove_datasets_if_not_teacher()
    RETURNS TRIGGER AS $$
        BEGIN
            SET search_path TO lensql;

            IF OLD.is_teacher = TRUE AND NEW.is_teacher = FALSE THEN
                DELETE FROM has_dataset WHERE username = OLD.username;
            END IF;
            RETURN NEW;
        END;
    $$ LANGUAGE plpgsql;

CREATE TRIGGER trg_remove_datasets_if_not_teacher
    BEFORE UPDATE ON users
    FOR EACH ROW
    WHEN (OLD.is_teacher = TRUE AND NEW.is_teacher = FALSE)
    EXECUTE FUNCTION remove_datasets_if_not_teacher();
